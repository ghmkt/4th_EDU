...
'''
판결문에서 어떤 토픽을 뽑아낼 수 있을지 예측을 전혀 하지 않고 텍스트를 넣었습니다.
오동건님의 퀘스트 답변을 보면 미리 토픽을 예측하고 프로그램을 돌리는 방법을 사용했는데 아주 괜찮은 접근같습니다.
토픽별로 정확히 어떤 의미를 담고있는지는 모르겠지만 의미 단위로 묶인다는 느낌이 있었습니다. 
어휘력이 부족해서 특정 단어로 토픽을 명명하긴 힘들었지만 나름대로 의미를 부여했습니다.

LDA 결과는 아래와 같습니다.
1. Topic별 단어 (weight > 4)
...
2.문서별 Topic은 사실 큰 의미가 없어서 첨부하지 않았습니다.
텍스트 파일은 카톡으로 보냈습니다!
'''        
