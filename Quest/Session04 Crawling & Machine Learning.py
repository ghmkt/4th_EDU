...
articles = []
for i in range(1,6):
    url = "https://news.naver.com/main/list.nhn?mode=LS2D&mid=shm&sid2=269&sid1=100&mid=shm&date=20181102&page=" + str(i)
    html = urlopen(url)
...
df = pd.DataFrame(columns = ['기사제목','내용'])

for i in range(len(url_list2)):
    html2 = urlopen(url_list2[i])
    ...
    df = df.append({'Title':title, 'Article':result}, ignore_index = True())
